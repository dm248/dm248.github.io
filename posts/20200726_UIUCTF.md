...
and *then* use a network to recognize it... But it turns out that one network can do [all of that] 
(https://srikavin.me/blog/posts/5f1c3dfcd7e47a02e27842ca-solving-minecraft-captchas-with-neural-networks)!
directly. Wow.
...
* 2) get the encryption of any string you specify (but no newlines or characters above 0x80)
...
By sending in plaintexts \x01, \x02, \0x03, ... one could verify that this was indeed so 
...
So this all suggested a constrained ECDLOG problem where *k* corresponding to some ASCII
text (the flag), which may be breakable with enough wits and some resources. 
126 bits mean only up to 16 characters, so as a first step I ruled out any such string
of the form *uiuctf{...}*. Next I thought
...
Indeed, making lots of connections, one could verify that with nonnegligible probability
...
without the cosmic ray bug (solving the 128-bit DLOG would not have worked at all) :/
